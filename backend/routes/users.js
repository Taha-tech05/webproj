...
router.patch(
    ...
    async (req, res) => {
        ...
        try {
            const result = await db.query(
                'UPDATE users SET role = $1 WHERE user_id = $2 RETURNING user_id as id, name, email, role, is_active',
                [role, id]
            );
...
router.patch(
    ...
    async (req, res) => {
        ...
        try {
            const result = await db.query(
                'UPDATE users SET is_active = $1 WHERE user_id = $2 RETURNING user_id as id, name, email, role, is_active',
                [is_active, id]
            );
            if (result.rows.length === 0) return res.status(404).json({ error: 'User not found.' });
            return res.json({ user: result.rows[0] });
